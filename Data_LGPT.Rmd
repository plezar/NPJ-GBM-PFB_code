...
```{r}
root_dir = "/Volumes/TOSHIBA/TIMElab/SCPCP000002"
...
```{r}
saveRDS(seurat_obj, "/Volumes/TOSHIBA/TIMElab/SCPCP000002/seurat_obj.rds")
```
...
```{r}
fig_path = "/Volumes/TOSHIBA/TIMElab/SCPCP000002"
...
```{r}
seurat_obj_sub = subset(seurat_obj, idents = c("Fibroblast"))
```
...
```{r}
seurat_obj_sub <- FindNeighbors(seurat_obj_sub, dims = 1:7)
seurat_obj_sub <- FindClusters(seurat_obj_sub, resolution = 0.5)
seurat_obj_sub <- RunUMAP(seurat_obj_sub, dims = 1:7)
Idents(seurat_obj_sub) <- "cell.type"
DimPlot(seurat_obj_sub)
```

```{r}
saveRDS(seurat_obj_sub, "/Volumes/TOSHIBA/TIMElab/SCPCP000002/stromal_cells.rds")
```
...
```{r}
mtx = read_tsv("/Volumes/TOSHIBA/TIMElab/SCPCP000002/bulk_quant.tsv") %>% column_to_rownames("gene_id")
```
...
```{r}
bulk_metadata = read_tsv("/Volumes/TOSHIBA/TIMElab/SCPCP000002/bulk_metadata.tsv") %>% column_to_rownames("sample_id")
...
```{r}
saveRDS(dge, "/Volumes/TOSHIBA/TIMElab/SCPCP000002/bulk_DGEList.rds")
```
...
```{r}
load("/Volumes/TOSHIBA/TIMElab/SCPCP000002/SCIPAC_data.RData")
seurat_obj = readRDS("/Volumes/TOSHIBA/TIMElab/SCPCP000002/seurat_obj.rds")
```
...
p = DimPlot(seurat_obj,
        group.by = "sig",
        cols = c("#ededed")) + NoLegend()
p
```

```{r}
ggsave(p, path="/Volumes/TOSHIBA/TIMElab/SCPCP000002", filename= "LGPT_SCIPAC.pdf", width=5.1, height=5.1, dpi=700)
```
...
```{r}
ggsave(p, path="/Volumes/TOSHIBA/TIMElab/SCPCP000002", filename= "LGPT_UMAP.pdf", width=5.1, height=5, dpi=700)
```